<script>
    ...
    import { selectedUpgradeNFT } from '$lib/stores/upgradeStore';
    ...
    // Convert $: to $derived
    const availableRarities = $derived(
        $selectedUpgradeNFT ? getAvailableRarities($selectedUpgradeNFT.rarity) : []
    );

    // Function to get available rarities based on current NFT rarity
    function getAvailableRarities(currentRarity) {
        const rarityOrder = ["common", "uncommon", "rare", "epic", "legendary"];
        const currentIndex = rarityOrder.indexOf(currentRarity);
        return rarity.filter((r, index) => rarityOrder.indexOf(r.value) > currentIndex);
    }

    // Modified selectNFT function to set initial values
    function selectNFT(nft) {
        $selectedUpgradeNFT = nft;
        valueType = nft.rarity;
    }
...
>
    {#if $selectedUpgradeNFT}
        <!-- Form View -->
        <div class="flex flex-col gap-4">
            <div class="flex justify-between items-center">
                <div class="text-3xl font-body">Upgrade NFT</div>
                <div class="flex items-center gap-4">
                    <div class="text-sm text-zinc-400">
                        Current: {$selectedUpgradeNFT.rarity}
                    </div>
                    <button 
                ...
                <div class="flex flex-col w-full">
                    ...
                    <Select.Root type="single" name="favoriteFruit" bind:value>
                        ...
                        <Select.Content class="rounded-none">
                            <Select.Group>
                                {#each availableRarities as rare}
                                    <Select.Item 
                                        class="rounded-none"
                                        value={rare.value}
            ...
            {#if value}
                <!-- Only show new form fields based on the difference between current and selected rarity -->
                {#if $selectedUpgradeNFT.rarity === "epic" && value === "legendary"}
                    <div class="flex flex-col gap-1 w-full">
                        ...
                        <Textarea 
                            class="rounded-none"
                            placeholder="Relaxed with a subtle smile."
                        />
                    </div>
                {:else if $selectedUpgradeNFT.rarity === "rare" && (value === "epic" || value === "legendary")}
                    <div class="flex flex-col gap-1 w-full">
                        <div class="font-body text-xs text-zinc-300 text-start">
                            Accessories
                        </div>
                    ...
                    {/if}
                {:else if $selectedUpgradeNFT.rarity === "uncommon" && (value === "rare" || value === "epic" || value === "legendary")}
                    <div class="flex flex-col gap-1 w-full">
                        <div class="font-body text-xs text-zinc-300 text-start">
                            Outfit
                        </div>
                    ...
                    {/if}
                {:else if $selectedUpgradeNFT.rarity === "common"}
    ...
    <div class="flex flex-col gap-2">
        <Button
            ...
            disabled={!$selectedUpgradeNFT || !value}
        >
            Upgrade for 0.05
            <svg
